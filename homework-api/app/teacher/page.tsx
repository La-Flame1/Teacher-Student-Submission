...
export default function TeacherPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  useEffect(() => {
    const plainParams = Object.fromEntries(Object.entries(searchParams || {}).map(([k, v]) => [k, String(v ?? '')]));
    setFilters((prev) => ({ ...prev, ...plainParams }));
  }, [searchParams]);
